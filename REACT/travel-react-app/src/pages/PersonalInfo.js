...
const PersonalInfo = () => {
 ...
 useEffect(() => {
   // localStorage에서 구글 사용자 정보 가져오기
   const userInfo = localStorage.getItem('userInfo');
   if (userInfo) {
     const googleUserInfo = JSON.parse(userInfo);
   }
       ...
       <div className="profile_wrapper ">
         <img
           ...
           src={user.userProfileImage 
             ? `http://localhost:9090${user.userProfileImage}` 
       ...
       <div className="personal_container">
         <div className="user-info">
           <div className="user-info-item">
             아이디 : {user.userId || user.email}
           </div>
           <div className="user-info-item">
             이름 : {user.userName || user.name}
           </div>
           <div className="user-info-item">
             닉네임 : {user.userNickName || user.name}
           </div>
         </div>
         ...
         <div className="button-container">
           <button 
             ...
             onClick={() => !user.googleId && openPopup('delete')}
             style={{ opacity: user.googleId ? 0.5 : 1 }}
           >
             계정탈퇴
           </button>
         </div>
